...
function makeDebouncedApplier<T>(
  ms: number,
...
describe("검색 debounce 타이밍", () => {
  test("여러 입력이 300ms 내에 연달아 오면 마지막 값만 반영", async () => {
    const applied: string[] = [];
    const schedule = makeDebouncedApplier<string>(80, (v) => applied.push(v));
  ...
  test("타이머 만료 전 cleanup 이면 apply 가 호출되지 않음", async () => {
    const applied: string[] = [];
    const schedule = makeDebouncedApplier<string>(80, (v) => applied.push(v));
...
function searchLinkHref(currentSearch: string): string {
  const trimmed = currentSearch.trim();
  return trimmed ? `/search?q=${encodeURIComponent(trimmed)}` : "/search";
}

describe("전체 검색 링크 URL", () => {
  test("검색어 있으면 /search?q=<encoded>", () => {
    assert.equal(searchLinkHref("정책"), "/search?q=%EC%A0%95%EC%B1%85");
    assert.equal(searchLinkHref("hello world"), "/search?q=hello%20world");
  });

  test("비어있으면 /search 로 이동 (q 없음)", () => {
    assert.equal(searchLinkHref(""), "/search");
    assert.equal(searchLinkHref("   "), "/search");
  });

  test("앞뒤 공백은 trim 후 전송", () => {
    assert.equal(searchLinkHref("  x  "), "/search?q=x");
  });
});
