...
exports.DOC_TYPE_ERROR_CODES = exports.DOC_TYPE_CODE_PATTERN = void 0;
exports.normalizeDocTypeCode = normalizeDocTypeCode;
exports.isValidDocTypeCode = isValidDocTypeCode;
exports.isFkMissingDocTypeError = isFkMissingDocTypeError;
exports.isDocTypeNotFoundError = isDocTypeNotFoundError;
exports.resolveDocTypeNotFoundHint = resolveDocTypeNotFoundHint;
const client_1 = require("@/lib/api/client");
exports.DOC_TYPE_CODE_PATTERN = /^[A-Z][A-Z0-9_-]*$/;
/**
...
function isValidDocTypeCode(normalized) {
    return exports.DOC_TYPE_CODE_PATTERN.test(normalized);
}
/**
 ...
 * 붙어도 동일하게 라우트되도록 함.
 */
function isFkMissingDocTypeError(message) {
    if (!message)
        return false;
 ...
 */
function isDocTypeNotFoundError(error) {
    if (error instanceof client_1.ApiError) {
        if (error.code === exports.DOC_TYPE_ERROR_CODES.DOC_TYPE_NOT_FOUND) {
            return true;
        }
 ...
 */
function resolveDocTypeNotFoundHint(error) {
    const allowed = new Set(["/admin/document-types"]);
    const fallback = {
        href: "/admin/document-types",
        label: "문서 유형 관리 열기",
    };
    if (error instanceof client_1.ApiError && error.hint) {
