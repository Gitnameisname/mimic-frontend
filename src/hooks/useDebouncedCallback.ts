...
// 함수 시그니처를 보존하기 위한 제너릭. 인자 타입은 그대로, 반환값은 무시한다.
// (debounce 의 결과는 비동기이므로 동기 반환값을 호출자에게 줄 수 없음.)
type AnyFn = (...args: never[]) => unknown;

export type DebouncedCallback<F extends AnyFn> = {
  /** 마지막 호출 시점부터 delay ms 뒤에 1회 실행 (이전 대기 호출은 취소). */
  (...args: Parameters<F>): void;
};

export type UseDebouncedCallbackResult<F extends AnyFn> = readonly [
  debounced: DebouncedCallback<F>,
  flush: () => void,
  cancel: () => void,
];
...
export function useDebouncedCallback<F extends AnyFn>(
  ...
  return useMemo(
    () => [debounced as DebouncedCallback<F>, flush, cancel] as const,
