...
export interface UseUserPreferencesResult {
  ...
  isLoading: boolean;
  isError: boolean;
  /** debounce 후 PATCH. 여러 번 호출하면 마지막 호출의 값만 전송. */
  updatePreference: (patch: UpdatePreferencesRequest) => void;
  /** 즉시 PATCH (debounce 생략). */
  updatePreferenceImmediate: (patch: UpdatePreferencesRequest) => void;
}
...
export function useUserPreferences(): UseUserPreferencesResult {
  ...
  const pendingRef = useRef<UpdatePreferencesRequest | null>(null);
  ...
  const mutation = useMutation({
    mutationFn: async (patch: UpdatePreferencesRequest) => {
      const res = await accountApi.updatePreferences(patch);
      return res.data;
    },
    onMutate: async (patch) => {
      ...
      if (previous) {
        const optimistic: UserPreferences = { ...previous };
    ...
    onError: (_err, _patch, ctx) => {
      ...
      toast(
        "뷰 설정을 저장하지 못했어요. 다음 접속에 복원되지 않을 수 있습니다.",
        "error",
      );
    },
    onSuccess: (next) => {
  ...
  const [schedulePending, flushPending, cancelPending] = useDebouncedCallback(
    firePending,
    PATCH_DEBOUNCE_MS,
  );
  ...
  const firePendingRef = useRef(firePending);
